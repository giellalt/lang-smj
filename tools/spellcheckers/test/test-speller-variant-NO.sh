...
passed=0
skipped=0
TYPOS="./typos-NO-generated.tsv"
ZHFST="../../../tools/spellcheckers/smj-NO.zhfst"
REPORT="../../../docs/typosreport/report-NO.json"
if test ! -f "$ZHFST" ; then
    printf "%sSKIP%s: NO (spellchecker not found)\n" "${light_blue}" "${reset}"
    skipped=$((skipped + 1))
    exit 77
fi
if test ! -f "$TYPOS" ; then
    printf "%sSKIP%s: NO (test data not found)\n" "${light_blue}" "${reset}"
    skipped=$((skipped + 1))
    exit 77
fi
