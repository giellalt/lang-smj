...
<!-- output method not used for the direct output -->
  ...
  <!-- Output -->
  <xsl:variable name="outDir" select="'_000_outDirTXT'"/>
  ...
  <xsl:template name="processFile">
    ...
    <xsl:value-of select="concat('frequencies - patterns:',$nl)"/>
	  <xsl:for-each select="$frequencyCells/pattern">
	    ...
	    <xsl:choose>
  	      <xsl:when test=".=''">
  	    <!--xsl:copy-of select="concat(normalize-space(./@frequency),$tab,' - &#40;EMPTY&#41;',$nl)"/-->
      ...
      <!-- output document -->
      <xsl:result-document href="{$outDir}/{$theName}_{$file_flag}.{$eTXT}" format="{$output_formatTXT}">
...
-->
      ...
      <xsl:message terminate="no">
	<xsl:value-of select="concat('   Done!','   Output directory/file:  ', $outDir, '/', $theName, '_', $file_flag, '.', $eTXT)"/>
      </xsl:message>
    </xsl:for-each>
